...
export interface FinanceFaqFields {
  question: string;
  answer: string;
  is_active: boolean;
}

export async function createFinanceFaq(): Promise<{
  ok?: true;
  id?: string;
  error?: string;
}> {
...
export async function updateFinanceFaq(
  id: string,
  fields: FinanceFaqFields,
...
export async function deleteFinanceFaq(
  id: string,
): Promise<{ ok?: true; error?: string }> {
  const { supabase, user, error } = await requireAdmin();
  if (error || !user) return { error: error ?? "Unauthorized" };
...
export async function reorderFinanceFaq(
  id: string,
  direction: "up" | "down",
): Promise<{ ok?: true; error?: string }> {
  const { supabase, user, error } = await requireAdmin();
