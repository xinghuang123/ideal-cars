...
export interface FinanceBenefitFields {
  icon: string;
  title: string;
  description: string;
  is_active: boolean;
...
export async function createFinanceBenefit(): Promise<{
  ok?: true;
  id?: string;
  error?: string;
}> {
...
export async function updateFinanceBenefit(
  id: string,
  fields: FinanceBenefitFields,
...
export async function deleteFinanceBenefit(
  id: string,
): Promise<{ ok?: true; error?: string }> {
  const { supabase, user, error } = await requireAdmin();
  if (error || !user) return { error: error ?? "Unauthorized" };
...
export async function reorderFinanceBenefit(
  id: string,
  direction: "up" | "down",
): Promise<{ ok?: true; error?: string }> {
  const { supabase, user, error } = await requireAdmin();
  ...
  if (idx === -1) return { error: "Benefit not found." };
